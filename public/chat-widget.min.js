class LacoProChat {
    constructor(t={}) {
        this.options = {
            ...
            primaryColor: t.primaryColor || "#00C853",
            autoOpen: t.autoOpen || false,
            autoOpenDelay: t.autoOpenDelay || 11000,
            ...t
        };
        this.init();
    }
    ...
    init() {
        ...
        e.className = "tiempoespacio-chat-widget tiempoespacio-chat-button";
        e.innerHTML = "📱";
        document.body.appendChild(e);

        const s = document.createElement("div");
        s.className = "tiempoespacio-chat-window";
        s.innerHTML = `
        ...
        this.chatWindow = s;
        this.messagesContainer = s.querySelector(".tiempoespacio-chat-messages");
        this.input = s.querySelector("textarea");
        this.sendButton = s.querySelector(".tiempoespacio-chat-input button");
        this.closeButton = s.querySelector(".tiempoespacio-chat-close");
        this.loading = s.querySelector(".tiempoespacio-chat-loading");
        this.chatButton = e;
        ...
        this.addMessage("Hola 👋 ¿Cómo te puedo ayudar hoy?", "bot");
    ...
    addMessage(t, e) {
        const s = document.createElement("div");
        s.className = `tiempoespacio-chat-message tiempoespacio-chat-${e}`;
        if (e === "bot") {
            s.innerHTML = this.sanitizeHTML(t);
        } else {
            s.textContent = t;
        }
    ...
    sanitizeHTML(t) {
        // Primero limpiamos cualquier HTML existente para evitar inyecciones XSS
        let cleanText = t.replace(/<[^>]*>/g, '');
        
        // Si el texto contiene HTML ya formateado para WhatsApp, lo respetamos
        if (t.includes('class="whatsapp-link"')) {
            return t;
        }
        
        // Procesar enlaces de WhatsApp
        if (cleanText.includes('+56992322998')) {
            // Asegurarse de que el número esté formateado correctamente
            cleanText = cleanText.replace(/\+56992322998/g, '<span class="whatsapp-contact">+56992322998</span>');
        }
        
        // Asegurarse de que el enlace de WhatsApp esté formateado correctamente
        cleanText = cleanText.replace(/https:\/\/wa\.me\/\+56992322998/g, 
            '<a href="https://wa.me/+56992322998" class="whatsapp-link" target="_blank">💬 Contactar por WhatsApp</a>');
        
        // Procesar otros enlaces
        cleanText = cleanText.replace(/(https?:\/\/(?!wa\.me)[^\s\n]+)/g, url => {
            if (url.includes('lacopro.cl/producto/')) {
                return `<a href="${url}" class="product-link" target="_blank">${url}</a>`;
            }
    ...
    async sendMessage() {
        ...
        try {
            // Verificar si es una consulta de WhatsApp
            if (['whatsapp', 'wsp', 'contacto', 'numero', 'teléfono', 'telefono'].some(k => t.toLowerCase().includes(k))) {
                const whatsappNumber = '+56992322998';
                const whatsappUrl = `https://wa.me/${whatsappNumber}`;
                // Crear el mensaje con HTML directamente para garantizar el hipervínculo
                const whatsappMessage = `Puedes contactarnos directamente:<br><br><span class="whatsapp-contact">${whatsappNumber}</span><br><br><a href="${whatsappUrl}" class="whatsapp-link" target="_blank">💬 Contactar por WhatsApp</a>`;
                
                this.addMessage(whatsappMessage, "bot");
            } else {
                const e = await fetch("https://lacopro-chatbot.onrender.com/chat", {
                    method: "POST",
                ...
                reply = reply.replace(/https:\/\/wa\.me\/https:\/\/wa\.me\/\+56992322998/g, 'https://wa.me/+56992322998');
...
window.LacoProChat = LacoProChat; 