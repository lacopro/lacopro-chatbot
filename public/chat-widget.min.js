...
this.messagesContainer.appendChild(s),this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight}sanitizeHTML(t){
    // Eliminar cualquier atributo HTML o formato markdown que pueda causar problemas
    t = t.replace(/target="_blank"/g, '');
    t = t.replace(/rel="noopener noreferrer"/g, '');
    t = t.replace(/class="whatsapp-link"/g, '');
    t = t.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '$1: $2');
    
    // Simplificar: primero limpiar cualquier enlace malformado de WhatsApp
    t = t.replace(/(https:\/\/wa\.me\/\+[0-9]+)(?:\s+[^<]+|<[^>]+>)+\1/g, '$1');
    
    // Convertir números de teléfono de WhatsApp directamente
    t = t.replace(/\+56992322998/g, '<a href="https://wa.me/+56992322998" class="whatsapp-link">+56992322998</a>');
    
    // Convertir enlaces WhatsApp (primero para evitar procesamiento doble)
    t = t.replace(/(https:\/\/wa\.me\/\+[0-9]+)/g, (match) => {
        return `<a href="${match}" class="whatsapp-link">${match}</a>`;
    });
    
    // Convertir URLs en enlaces clickeables (después de WhatsApp)
