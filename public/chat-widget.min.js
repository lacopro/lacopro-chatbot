...
this.messagesContainer.appendChild(s),this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight}sanitizeHTML(t) {
    // Paso 1: Limpiar el texto de cualquier HTML malicioso
    t = t.replace(/<[^>]*>/g, '');
    
    // Paso 2: Procesar enlaces de WhatsApp - versión corregida
    const whatsappNumber = '+56992322998';
    if (t.includes(whatsappNumber)) {
        // Primero reemplazamos cualquier formato incorrecto del enlace de WhatsApp
        t = t.replace(/https:\/\/wa\.me\/\+?\d+/g, `https://wa.me/${whatsappNumber}`);
        
        // Reemplazar el número de teléfono con formato limpio
        t = t.replace(new RegExp(whatsappNumber, 'g'), 
            `<span class="whatsapp-contact">${whatsappNumber}</span>`);
        
        // Reemplazar el enlace de WhatsApp completo
        t = t.replace(`https://wa.me/${whatsappNumber}`, 
            `<a href="https://wa.me/${whatsappNumber}" class="whatsapp-link" target="_blank">💬 Contactar por WhatsApp</a>`);
    }
    
    // Paso 3: Procesar enlaces web normales (que no sean WhatsApp)
    t = t.replace(/(https?:\/\/(?!wa\.me)[^\s]+)/g, (url) => {
        // Verificar si es un enlace de producto de Lacopro
