class TiempoespacioChat{constructor(t={}){this.options={position:t.position||"bottom-right",primaryColor:t.primaryColor||"#00C853",autoOpen:t.autoOpen||false,autoOpenDelay:t.autoOpenDelay||11000,...t},this.init()}init(){const t=document.createElement("style");t.textContent=`
...
// Inicializar el botón inmediatamente
const chatButton = document.querySelector(".tiempoespacio-chat-button");
if(chatButton) {
    chatButton.onclick = () => this.toggleChat();
}

// Configurar apertura automática si está habilitada
...
}toggleChat(){this.chatWindow.style.display="none"===this.chatWindow.style.display?"block":"none"}addMessage(t,e){const s=document.createElement("div");s.className=`tiempoespacio-chat-message tiempoespacio-chat-${e}`;
...
this.messagesContainer.appendChild(s),this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight}sanitizeHTML(t){
    ...
    // Eliminar paréntesis y URL si quedaron
