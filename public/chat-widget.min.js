...
.tiempoespacio-chat-message a.whatsapp-link{color:#25D366;font-weight:bold;text-decoration:underline;display:inline-block;padding-left:22px;background:url('https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/WhatsApp.svg/20px-WhatsApp.svg.png') no-repeat left center;background-size:18px}
.tiempoespacio-chat-message a.whatsapp-link:hover{opacity:0.8}
...
@keyframes dots{0%,20%{content:'.'} 40%{content:'..'} 60%{content:'...'} 80%,100%{content:''}}`,document.head.appendChild(t);const e=document.createElement("button");e.className="tiempoespacio-chat-widget tiempoespacio-chat-button",e.innerHTML="📱",document.body.appendChild(e);const s=document.createElement("div");s.className="tiempoespacio-chat-window",s.innerHTML=`
...
// Configurar apertura automática
...
this.messagesContainer.appendChild(s),this.messagesContainer.scrollTop=this.messagesContainer.scrollHeight}sanitizeHTML(t){
    // Limpieza profunda para URLs de WhatsApp duplicadas y atributos visibles
    t = t.replace(/https:\/\/wa\.me\/https:\/\/wa\.me\//, 'https://wa.me/');
    t = t.replace(/target="_blank"/g, '');
    t = t.replace(/rel="noopener noreferrer"/g, '');
    t = t.replace(/class="whatsapp-link"/g, '');
    
    // Eliminar completamente cualquier atributo HTML visible
    t = t.replace(/\s+class="[^"]*"/g, '');
    t = t.replace(/\s+rel="[^"]*"/g, '');
    t = t.replace(/\s+target="[^"]*"/g, '');
    
    // Extraer y limpiar completamente cualquier URL de WhatsApp antes de procesarla
    const whatsappPattern = /(https:\/\/wa\.me\/\+[0-9]+)/g;
    const matches = t.match(whatsappPattern);
    
    if (matches) {
        // Reemplazar cada URL de WhatsApp con una versión limpia
        matches.forEach(match => {
            // Eliminar la URL actual y cualquier atributo o texto entre ella
            const cleanUrl = match.replace(/['"<>]/g, '');
            const escapedMatch = match.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const regexPattern = new RegExp(`${escapedMatch}[^<>]*${escapedMatch}|${escapedMatch}`, 'g');
            
            // Reemplazar con un enlace limpio
            t = t.replace(regexPattern, `<a href="${cleanUrl}" class="whatsapp-link">+56992322998</a>`);
        });
    }
    
    // Detectar y convertir directamente el número de WhatsApp de Lacopro
    t = t.replace(/\+56992322998/g, '<a href="https://wa.me/+56992322998" class="whatsapp-link">+56992322998</a>');
    
    // Convertir otras URLs en enlaces clickeables
    t = t.replace(/(https?:\/\/(?!wa\.me)[^\s<>"']+)/g, (match) => {
        // Evitar procesar enlaces ya convertidos
        if (match.includes('<a href') || match.includes('class=')) return match;
    ...
    // Pre-procesar la respuesta para limpiar URLs incorrectas de WhatsApp antes de añadirla
