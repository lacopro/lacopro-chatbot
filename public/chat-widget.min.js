class TiempoespacioChat{constructor(t={}){if(!t.serverUrl)throw new Error("Es necesario especificar la URL del servidor (serverUrl)");this.options={position:t.position||this.getScriptParam("position")||"bottom-right",primaryColor:t.primaryColor||this.getScriptParam("color")||"#00C853",serverUrl:t.serverUrl,...t},this.init()}getScriptParam(param){const scripts=document.getElementsByTagName("script");const currentScript=scripts[scripts.length-1];const url=new URL(currentScript.src);return url.searchParams.get(param)}init(){const t=document.createElement("style");t.textContent=`
...
.tiempoespacio-chat-input button:hover{background:#ff4da6}
.tiempoespacio-chat-loading{display:none;text-align:center;padding:10px}
.tiempoespacio-chat-loading::after{content:'...';animation:dots 1.5s steps(4,end) infinite}
@keyframes dots{0%,20%{content:'.'} 40%{content:'..'} 60%{content:'...'} 80%,100%{content:''}}`,document.head.appendChild(t);const e=document.createElement("button");e.className="tiempoespacio-chat-widget tiempoespacio-chat-button",e.innerHTML="📱",e.onclick=()=>this.toggleChat(),document.body.appendChild(e);const s=document.createElement("div");s.className="tiempoespacio-chat-window",s.innerHTML=`
